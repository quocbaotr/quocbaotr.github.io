...
/* Full Screen */
// This function attempts to enter fullscreen mode for a given element.
function enterFullScreen(element) {
  // Check if the browser supports the standard requestFullscreen method.
  if (element.requestFullscreen) {
    element.requestFullscreen();  // Request fullscreen mode.
  }
  // Check if the browser supports the Firefox-specific mozRequestFullScreen method.
  else if (element.mozRequestFullScreen) {
    element.mozRequestFullScreen(); // Request fullscreen mode in Firefox.
  }
  // Check if the browser supports the WebKit-specific webkitRequestFullscreen method (used in Safari).
  else if (element.webkitRequestFullscreen) {
    element.webkitRequestFullscreen(); // Request fullscreen mode in Safari.
  }
  // Check if the browser supports the Microsoft-specific msRequestFullscreen method (used in IE/Edge).
  else if (element.msRequestFullscreen) {
    element.msRequestFullscreen(); // Request fullscreen mode in Internet Explorer/Edge.
  }
}

// Get a reference to the button element with the ID "full_screen".
...
btn.addEventListener("click", function() {
  ...
  // Call the enterFullScreen function to request fullscreen mode for the video element.
  enterFullScreen(videoEle);
});

// Listen for the fullscreenchange event, which fires when the fullscreen mode is entered or exited.
document.addEventListener('fullscreenchange', (event) => {
  if (document.fullscreenElement) {
    // If a fullscreen element exists, log that fullscreen mode has been entered.
    console.log('Entered fullscreen:', document.fullscreenElement);
  } else {
    // If there's no fullscreen element, log that fullscreen mode has been exited.
